//
//  Colors.h
//  CS51DigitalSignalProcessor
//
//  Created by Gabe Montague on 4/23/15.
//
//

#ifndef CS51DigitalSignalProcessor_Colors_h
#define CS51DigitalSignalProcessor_Colors_h

#define windowWidth 800
#define windowHeight 600

// Define some colors to use in the GUI
#define textColor Colours::white
#define graphBgColor Colour::fromRGB(15, 20, 80)
#define graphPointColor Colour::fromRGB(255, 255, 255)
#define graphPixelOffset 1
#define graphBaseLineEmptyColor Colours::red

// Define point size for graphs
#define graphPointSize 2

#endif
